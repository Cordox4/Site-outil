...
export const tools = {
  'summarize': {
    name: 'Résumé automatique', icon: 'fileText', desc: 'Résumé extractif d\'un texte (les phrases clés).', cat: 'ai',
    render(root) {
  ...
  'translate': {
    name: 'Traducteur', icon: 'globe', desc: 'Traduisez un texte (via MyMemory).', cat: 'ai', badge: 'Live',
    render(root) {
      const p = panel(); root.append(p);
  ...
  'language-detect': {
    name: 'Détecteur de langue', icon: 'message', desc: 'Devinez la langue d\'un texte.', cat: 'ai',
    render(root) {
      const p = panel(); root.append(p);
  ...
  'paraphrase': {
    name: 'Reformulation IA', icon: 'refresh', desc: 'Reformulez un texte avec l\'IA.', cat: 'ai', badge: 'API',
