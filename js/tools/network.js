...
export const tools = {
  'ip-lookup': {
    name: 'IP Lookup', icon: 'globe', desc: 'Géolocalisez une adresse IP (ou la vôtre).', cat: 'network',
    render(root) {
  ...
  'dns-lookup': {
    name: 'DNS Lookup', icon: 'search', desc: 'Interrogez les enregistrements DNS d\'un domaine.', cat: 'network',
    render(root) {
      const p = panel(); root.append(p);
  ...
  'domain-checker': {
    name: 'Nom de domaine checker', icon: 'globe', desc: 'Vérifiez si un domaine est enregistré (RDAP).', cat: 'network',
    render(root) {
      const p = panel(); root.append(p);
  ...
  'whois': {
    name: 'Whois Lookup', icon: 'list', desc: 'Informations d\'enregistrement d\'un domaine (RDAP).', cat: 'network',
    render(root) {
      const p = panel(); root.append(p);
      const dom = el('input', { type: 'text', value: 'example.com' });
      const out = el('div'); const st = el('div');
  ...
  'ping-tester': {
    name: 'Ping Tester', icon: 'globe', desc: 'Mesurez la latence HTTP vers un site.', cat: 'network',
    render(root) {
      const p = panel(); root.append(p);
  ...
  'http-headers': {
    name: 'HTTP Headers Checker', icon: 'server', desc: 'Inspectez les en-têtes HTTP d\'une URL.', cat: 'network',
    render(root) {
      const p = panel(); root.append(p);
