...
export const tools = {
  ...
  'temperature': {
    name: 'Convertisseur de température', icon: 'thermometer', desc: 'Celsius, Fahrenheit et Kelvin.', cat: 'convert',
    render(root) {
      const p = panel(); root.append(p);
  ...
  'currency': {
    name: 'Convertisseur de devises', icon: 'exchange', desc: 'Taux de change du jour (via Frankfurter).', cat: 'convert', badge: 'Live',
    render(root) {
      const p = panel(); root.append(p);
  ...
  'timezones': {
    name: 'Fuseaux horaires', icon: 'clock', desc: 'Heure actuelle dans plusieurs villes.', cat: 'convert',
    render(root) {
      const p = panel(); root.append(p);
