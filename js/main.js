...
import { el, adInline, icon } from './ui.js';
...
function setTheme(t) {
  ...
  themeBtn.innerHTML = '';
  themeBtn.append(icon(t === 'dark' ? 'sun' : 'moon'));
...
function toolCard(t) {
  ...
  return el('a', {
    ...
    el('span', { class: 't-icon' }, icon(t.icon || 'tool')),
...
function renderHome() {
  ...
  categories.forEach((c, i) => {
    ...
    content.append(el('h2', { class: 'section-title' },
      el('span', { class: 'emoji' }, icon(c.icon)), c.name,
...
function renderCategory(catId) {
  ...
  content.append(el('div', { class: 'tool-header' },
    ...
    el('h1', {}, el('span', {}, icon(c.icon)), ' ' + c.name)));
  const grid = el('div', { class: 'tool-grid' });
...
async function renderTool(id) {
  ...
  content.append(el('div', { class: 'tool-header' },
    el('div', { class: 'breadcrumb' },
      ...
      ' / ' + t.name),
    el('h1', {}, el('span', {}, icon(t.icon || 'tool')), ' ' + t.name),
...
function search(q) {
  ...
  matches.forEach(t => searchResults.append(
    el('a', { href: '#/tool/' + t.id, onclick: () => { searchResults.hidden = true; searchInput.value = ''; } },
      el('span', { class: 'sr-icon' }, icon(t.icon || 'tool')), ' ' + t.name,
