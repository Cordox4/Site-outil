...
// Reusable ad banner (placeholder) — replace data-ad slot with your ad network code.
export function adInline(id = 'inline') {
  return el('div', { class: 'ad-slot ad-inline', 'data-ad': id },
    el('span', { class: 'ad-label' }, 'Emplacement publicitaire'));
}
...
export function adInterstitial() {
  return new Promise((resolve) => {
    ...
    btn.disabled = true; cd.textContent = n;
    const timer = setInterval(() => {
      n--; cd.textContent = n;
      if (n <= 0) { clearInterval(timer); btn.disabled = false; btn.innerHTML = 'Continuer'; }
    }, 1000);
    const onClick = () => {
      clearInterval(timer);
